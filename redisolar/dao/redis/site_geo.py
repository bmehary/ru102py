...
class SiteGeoDaoRedis(SiteGeoDaoBase, RedisDaoBase):
    ...
    def _find_by_geo_with_capacity(self, query: GeoQuery, **kwargs) -> Set[Site]:
        ...
        # for site_id in site_ids:
        #    scores[site_id] = self.redis.zscore(self.key_schema.capacity_ranking_key(), site_id)

        # with pipelines
        for site_id in site_ids:
            p.zscore(self.key_schema.capacity_ranking_key(), site_id)

        site_scores = p.execute()

        # using for-loop manually creating index
        # for i in range(len(site_ids)):
        #    scores[site_ids[i]] = site_scores[i]

        # using for-loop with enumerate
        for i, site_id in enumerate(site_ids):
            scores[site_id] = site_scores[i]
